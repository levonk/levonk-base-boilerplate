...
- [ ] Add `include_privacy` boolean option to `copier.yml` — `apps/cli/python/core/copier.yml`
- [ ] Create `privacy.py.jinja` template file with privacy mode implementation — `apps/cli/python/core/files/{{project_slug}}/privacy.py.jinja`
- [ ] Implement explicit ignore lists for sensitive data — `privacy.py.jinja`
- [ ] Distinguish between "unknown" (logged but not assigned) and "anonymous" (ignored entirely) — `privacy.py.jinja`
- [ ] Add configurable privacy toggles to disable specific data collection — `privacy.py.jinja`
- [ ] Add privacy settings to config file schema — `apps/cli/python/core/files/{{project_slug}}/config.py.jinja`
- [ ] Add `--privacy-mode` flag to main command in `__main__.py.jinja` — `apps/cli/python/core/files/{{project_slug}}/__main__.py.jinja`
- [ ] Integrate privacy mode into data collection logic — `__main__.py.jinja`
- [ ] Add tests for privacy mode with ignore lists — `apps/cli/python/core/files/tests/test_privacy.py.jinja` (conditional)
- [ ] Add tests for unknown vs anonymous distinction — `tests/test_privacy.py.jinja` (conditional)
...
- [ ] Privacy mode implemented with explicit ignore lists
- [ ] "Unknown" data logged but not assigned
- [ ] "Anonymous" data ignored entirely
...
- [ ] All tests pass for privacy mode scenarios
