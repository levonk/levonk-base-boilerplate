...
- [x] Implement TTY detection using `golang.org/x/term` package
...
- [x] Add auto-detection logic: agent mode when no TTY OR agent session detected; human mode when TTY present AND no agent session
