...
- **Feature name:** Python CLI AXI (Agent eXperience Interface) Support
- **Summary:** Implementation of agent mode standards following the AXI specification to enable autonomous AI agents to efficiently interact with Python CLI tools via shell execution, while preserving human-friendly features.
- **Context:**
  - This feature extends the Python CLI boilerplate to support agent mode as defined in ADR-20260607001 (CLI Tool Standards v4.0.0)
...
## Goals
- Enable autonomous AI agents to efficiently use Python CLI tools with minimal token consumption
...
### As an AI agent using a Python CLI tool
...
### As a developer using the Python CLI boilerplate
...
### As a DevOps engineer deploying Python CLI tools
...
5. **TOON Format Specification**
   ...
   - Implement TOON encoder/decoder for Python
...
21. **Agent Skill Support**
    ...
    - Rewrite command examples to non-interactive form (e.g., `python -m mytool ...`)
...
- **TOON Library**: Implement custom TOON encoder/decoder or find PyPI package
...
- **Dependencies**: Leverage existing Python ecosystem (click, typer, argparse) where possible
...
- Existing Python CLI boilerplate - must be extended, not replaced
- Core CLI standards implementation (20260707-cli-python-standards) - should be complete first
