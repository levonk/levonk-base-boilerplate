...
- [ ] Implement `docker-compose.yml` with dynamic services based on `number_of_worktrees` (controller + agents):
  ...
  - [ ] Note: Jinja template added at `docker-compose.yml.j2` implementing dynamic agents, mounts, users, env files, and IDE port logic. Repo holds minimal placeholder `docker-compose.yml` for linting. Pending: AGENT_PORT example and healthchecks.
