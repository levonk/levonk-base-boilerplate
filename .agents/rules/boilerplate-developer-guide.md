...
1. **Write shared content to `_shared/`** — this is the single source of truth. Shared files live at the root of `_shared/` (e.g. `_shared/dot_npmrc.jinja`) or in `_shared/partials/nx-partials/` (for Nx target partials).
...
3. **Templates reference shared partials via Jinja includes**: `{% include "partials.bak/dot_npmrc.jinja" %}`.
...
```jinja
{% include "partials.bak/dot_npmrc.jinja" %}
```

The shared partial at `_shared/dot_npmrc.jinja` enforces supply-chain hygiene and reproducibility settings (`save-exact`, `min-release-age`, `engine-strict`, `audit-signature-strict`, etc.). To change `.npmrc` behavior across all templates, edit the shared partial — do not duplicate settings in individual template `.npmrc.jinja` files.
